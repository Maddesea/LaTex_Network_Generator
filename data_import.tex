...
\ifluatex
    ...
    \directlua{
        ...
        function loadJSONNetwork(filename)
            ...
            -- NOTE: Full JSON parsing not yet implemented
            -- For production use, install a proper JSON library like lunajson:
            --   luarocks install lunajson
            -- Then replace this stub with: local json = require("lunajson")

            tex.print("\\PackageWarning{data_import}{JSON parsing is not fully implemented. " ..
                     "Use CSV import for production. See data_import.tex line 76 for implementation notes.}{}")
            tex.print("\\message{JSON file loaded but not parsed: " .. filename .. "}")
        end
        ...
        function parseJSONNodes(jsonData)
            ...
            -- TODO: Implementation requires proper JSON library (lunajson)
            -- Once implemented, should call iputils.registerNode() for each node
            tex.print("\\PackageWarning{data_import}{JSON node parsing not implemented}{}")
        end
    }
...
\newcommand{\createNodeFromType}[6]{%
    ...
    % Register node for subnet zone generation
    \registerNodeForSubnet{#2}{#3}%
}
    ...
    \directlua{
        ...
        function nmap.parseXML(filename)
            ...
            for i, host in ipairs(hosts) do
                ...
                -- Register node for subnet zone generation
                iputils.registerNode(nodeId, host.ip)
            end
...
\ifluatex
    \directlua{
        ...
        function yaml.parse(filename)
            ...
            -- NOTE: Full YAML parsing not yet implemented
            -- For production use, install a proper YAML library like lyaml:
            --   luarocks install lyaml
            -- Then replace this stub with: local yaml = require("lyaml")

            tex.print("\\PackageWarning{data_import}{YAML parsing is not fully implemented. " ..
                     "Use CSV import for production. See data_import.tex line 420 for implementation notes.}{}")
            tex.print("\\message{YAML file loaded but not parsed: " .. filename .. "}")
        end
        ...
        function yaml.loadNetwork(filename)
            ...
            -- TODO: Implementation requires proper YAML library (lyaml)
            -- Once implemented, should call iputils.registerNode() for each node
            yaml.parse(filename)
        end
    }
...
\ifluatex
    ...
    \newcommand{\registerNodeForSubnet}[2]{%
        % #1=nodeId, #2=ip
        % Register node for automatic subnet zone generation
        \directlua{iputils.registerNode("#1", "#2")}%
    }
...
\else
    ...
    \newcommand{\registerNodeForSubnet}[2]{}% No-op if LuaTeX not available
    \newcommand{\autoGenerateSubnetZones}{}
\fi
        ...
        function nessus.parseFile(filename)
            ...
            for hostBlock in content:gmatch("<ReportHost.-</ReportHost>") do
                ...
                if hostIP then
                    ...
                    -- Register node for subnet zone generation
                    iputils.registerNode(nodeId, hostIP)
                end
            end
...
% Example: Import from Nmap XML (LuaTeX required)
% \importNmapXML{data/nmap-scan.xml}

% Example: Import from Nessus (LuaTeX required)
% \importNessusXML{data/nessus-scan.nessus}

% Example: Import from JSON (LuaTeX required - NOT YET FULLY IMPLEMENTED)
% \importNetworkFromJSON{data/network.json}

% Example: Import from YAML (LuaTeX required - NOT YET FULLY IMPLEMENTED)
% \importNetworkFromYAML{data/network.yaml}

% Example: Auto-generate subnet zones (LuaTeX required)
% NOTE: Subnet zones are automatically generated when using:
%   - CSV import (importNodesFromCSV, importNodesAutoPositioned)
%   - Nmap import (importNmapXML)
%   - Nessus import (importNessusXML)
% All imported nodes are automatically registered by IP subnet.
% To draw the subnet zones, call:
...
%   \autoGenerateSubnetZones  % Automatically groups nodes by /24 subnet
% \end{scope}
...
% - [x] CSV import with validation - IMPLEMENTED
% - [x] Nessus scan integration (.nessus file parser) - IMPLEMENTED
% - [x] Nmap XML import - IMPLEMENTED
% - [x] Auto-position calculation for imported nodes - IMPLEMENTED
% - [x] Subnet detection from IP addresses - IMPLEMENTED
% - [x] Auto-registration of nodes for subnet tracking - IMPLEMENTED v1.2
% - [x] Service-to-node-type mapping - IMPLEMENTED
% - [ ] Enhanced JSON/YAML with proper library (lunajson/lyaml) - STUB ONLY
%       Current implementation is a stub with warnings
...
% Low Priority - Integration & Scalability:
...
%        - Enhanced validation
%
% v1.2 - Bug fixes and improvements (2024-11-17)
%        CRITICAL BUG FIXES:
%        - Fixed subnet zone generation (was passing node ID instead of IP)
%        - Fixed connection type default fallback logic
%        - Improved CSV parser robustness (empty line handling)
%        - Added whitespace trimming to all CSV parsers
%
%        NEW FEATURES:
%        - Added automatic node registration for subnet tracking
%        - Integrated iputils.registerNode() into all import functions
%        - Added \registerNodeForSubnet LaTeX command
%        - Subnet zones now work automatically with CSV, Nmap, Nessus imports
%
%        IMPROVEMENTS:
%        - Better error messages for JSON/YAML stubs
%        - Added implementation notes for JSON/YAML parsers
%        - Enhanced documentation with subnet zone usage examples
%        - Updated TODO list to reflect current implementation status

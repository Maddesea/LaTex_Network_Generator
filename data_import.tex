...
% ============================================================================
% ADVANCED FILTERING AND TRANSFORMATION FUNCTIONS
% ============================================================================

\newcommand{\filterNodesBySubnet}[2]{%
    % Filter nodes by subnet/IP range
    % Usage: \filterNodesBySubnet{192.168.1}{jsonfile.json}
    \directlua{
        local file = io.open("#2", "r")
        if file then
            local content = file:read("*all")
            file:close()
            local data = json.decode(content)
...
\newcommand{\filterNodesByType}[2]{%
    % Filter nodes by type (server, firewall, router, etc.)
    % Usage: \filterNodesByType{server}{jsonfile.json}
    \directlua{
        local file = io.open("#2", "r")
        if file then
            local content = file:read("*all")
            ...
            if data and data.nodes then
                ...
                convertJSONToNodes({nodes = filtered})
            end
        end
    }
}

\newcommand{\filterVulnerabilitiesByCVSS}[2]{%
    % Filter and display only high-severity vulnerabilities
    % Usage: \filterVulnerabilitiesByCVSS{7.0}{jsonfile.json}
    \directlua{
        local file = io.open("#2", "r")
        if file then
            local content = file:read("*all")
            ...
            if data and data.threats then
                for i, threat in ipairs(data.threats) do
                    if threat.severity and threat.severity >= #1 then
                        -- Render high-severity threats only
                        if threat.cve then
                            tex.print(string.format("\\markVulnerability{%s}{%s}{%.1f}",
                                threat.target, threat.cve, threat.severity))
                        end
                    end
                end
            end
...
\newcommand{\transformIPsToLabels}{%
    % Transform IP addresses to be used as node labels
    % Useful for quick visualization
    \directlua{
        -- This is a placeholder for IP-to-label transformation
        -- Can be expanded to use DNS lookups or custom mappings
    }
}

\newcommand{\autoLayoutBySubnet}[1]{%
    % Automatically layout nodes by subnet grouping
    % Usage: \autoLayoutBySubnet{jsonfile.json}
    \directlua{
        local file = io.open("#1", "r")
        if file then
            local content = file:read("*all")
            ...
            if data and data.nodes then
                -- Group nodes by subnet (first 3 octets)
                local subnets = {}

                for i, node in ipairs(data.nodes) do
                    local ip = node.ip or "0.0.0.0"
                    local subnet = ip:match("^(%d+%.%d+%.%d+)%.%d+$") or "other"

                    if not subnets[subnet] then
                        subnets[subnet] = {}
                    end
                    table.insert(subnets[subnet], node)
                end

                -- Layout each subnet in a row
                local subnetY = 0
...
\newcommand{\mergeNetworkData}[2]{%
    % Merge two JSON network data files
    % Usage: \mergeNetworkData{file1.json}{file2.json}
    \directlua{
        local file1 = io.open("#1", "r")
        local file2 = io.open("#2", "r")

        if file1 and file2 then
            local content1 = file1:read("*all")
            ...
            if data1 and data2 then
                -- Merge nodes
                local merged = {nodes = {}, connections = {}, threats = {}}

                if data1.nodes then
                    for i, node in ipairs(data1.nodes) do
                        table.insert(merged.nodes, node)
                    end
                end

                if data2.nodes then
                    for i, node in ipairs(data2.nodes) do
                        table.insert(merged.nodes, node)
                    end
                end

                -- Merge connections
                if data1.connections then
                    for i, conn in ipairs(data1.connections) do
                        table.insert(merged.connections, conn)
                    end
                end

                if data2.connections then
                    for i, conn in ipairs(data2.connections) do
                        table.insert(merged.connections, conn)
                    end
                end

                -- Merge threats
                if data1.threats then
                    for i, threat in ipairs(data1.threats) do
                        table.insert(merged.threats, threat)
                    end
                end

                if data2.threats then
                    for i, threat in ipairs(data2.threats) do
                        table.insert(merged.threats, threat)
                    end
                end

                tex.print("\string\\renewcommand{\string\\renderNetworkNodes}{")
                convertJSONToNodes(merged)
                tex.print("}")

                tex.print("\string\\renewcommand{\string\\renderConnections}{")
                convertJSONToConnections(merged)
                tex.print("}")
            end
        end
    }
...
\newcommand{\excludeNodesByType}[2]{%
    % Exclude certain node types from rendering
    % Usage: \excludeNodesByType{client}{jsonfile.json}
    \directlua{
        local file = io.open("#2", "r")
        if file then
            local content = file:read("*all")
...
\newcommand{\scaleNodePositions}[2]{%
    % Scale all node positions by a factor
    % Usage: \scaleNodePositions{1.5}{jsonfile.json}
    \directlua{
        local file = io.open("#2", "r")
        if file then
            local content = file:read("*all")
            ...
            if data and data.nodes then
                local factor = tonumber("#1") or 1.0

                for i, node in ipairs(data.nodes) do
                    if node.x then node.x = node.x * factor end
                    if node.y then node.y = node.y * factor end
                end

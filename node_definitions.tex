...
% Configurable parameters for node styling and behavior
\newlength{\containerStackOffset}
\setlength{\containerStackOffset}{2pt}  % Offset between stacked container layers

% IP address generation defaults
\newcommand{\defaultIPOctetStart}{10}  % Default starting octet for auto-generated IPs

% Validation scoring parameters
\newcommand{\validationErrorPenalty}{10}  % Penalty points per validation error
\newcommand{\validationWarningPenalty}{5}  % Penalty points per validation warning
\newcommand{\ipConflictPenalty}{15}  % Penalty points per IP conflict (critical)
\newcommand{\portConflictPenalty}{8}  % Penalty points per port conflict
...
\newcommand{\registerNode}[3]{
    ...
    \IfStrEq{#1}{}{
        \PackageError{node_definitions}{Node ID cannot be empty}{}
    }{}
    \IfStrEq{#2}{}{
        \PackageError{node_definitions}{IP address cannot be empty for node #1}{}
    }{}
    \IfStrEq{#3}{}{
        \PackageError{node_definitions}{Hostname cannot be empty for node #1}{}
    }{}
...
% Returns: Node ID if found, empty string otherwise
\newcommand{\getNodeByIP}[2]{
    \StrSubstitute{#1}{.}{_}[\safeip]
    \pgfkeysifdefined{/nodemap/byip/\safeip}{
        \pgfkeysgetvalue{/nodemap/byip/\safeip}{#2}
    }{
        \def#2{} % Return empty if not found
    }
}
...
\newcommand{\getNodeByHostname}[2]{
    \pgfkeysifdefined{/nodemap/byhost/#1}{
...
% Returns: IP address if found, empty string otherwise
\newcommand{\getNodeIP}[2]{
    \pgfkeysifdefined{/nodemap/byid/#1/ip}{
...
% Returns: Hostname if found, empty string otherwise
\newcommand{\getNodeHostname}[2]{
    \pgfkeysifdefined{/nodemap/byid/#1/hostname}{
...
% Get node metadata (with error handling)
\newcommand{\getNodeType}[2]{
    \pgfkeysifdefined{/nodemap/byid/#1/type}{
...
\newcommand{\getNodeOS}[2]{
    \pgfkeysifdefined{/nodemap/byid/#1/os}{
...
\newcommand{\getNodeStatus}[2]{
    \pgfkeysifdefined{/nodemap/byid/#1/status}{
...
\newcommand{\getNodeCluster}[2]{
    \pgfkeysifdefined{/nodemap/byid/#1/cluster}{
...
% TODO: Advanced hash map features (future enhancements)
...
% Note: Error handling for missing keys is now implemented
...
\newcommand{\validateIPv4}[2]{
    \def#2{0} % Assume invalid by default (security best practice)
...
\newcommand{\validateOctet}[2]{
    ...
    % Check if input is numeric using xstring
    \IfInteger{#1}{
...
\newcommand{\validateIPv4Enhanced}[2]{
    ...
    \ifnum\dotcount=3\relax
        ...
        % Validate each octet using validateOctet (includes numeric check)
        \validateOctet{\octetA}{\validA}
        \validateOctet{\octetB}{\validB}
        \validateOctet{\octetC}{\validC}
        \validateOctet{\octetD}{\validD}

        % All octets must be valid
        \ifnum\validA=1\relax
            \ifnum\validB=1\relax
                \ifnum\validC=1\relax
                    \ifnum\validD=1\relax
                        \def#2{1} % Mark as valid if all octets are valid
                    \fi
                \fi
            \fi
        \fi
...
% Supports /8, /16, /24, /32; defaults to /24 for other values
\newcommand{\extractSubnetCIDR}[3]{
    ...
    \else
        ...
        \else
            ...
            \else
                \ifnum#2=32\relax
                    \def#3{\octetA.\octetB.\octetC.\octetD}
                \else
                    % Default to /24 for unsupported CIDR values
                    \PackageWarning{node_definitions}{Unsupported CIDR /#2, using /24 default}
                    \def#3{\octetA.\octetB.\octetC.0}
                \fi
            \fi
        \fi
...
\newcommand{\isPrivateIP}[2]{
    ...
    \ifthenelse{\equal{\octetA}{172}}{
        % Properly validate second octet is in range 16-31
        \ifnum\octetB<16\relax
            \def#2{0}
        \else
            \ifnum\octetB>31\relax
                \def#2{0}
            \else
                \def#2{1}
            \fi
...
% Format IP address text with consistent styling (avoids nested TikZ nodes)
% Usage: \iptext{192.168.1.10}
\newcommand{\iptext}[1]{%
    {\scriptsize\ttfamily\textcolor{black!70}{#1}}%
}

% Format IP address with custom background color
% Usage: \iptextcolored{192.168.1.10}{threatCritical!20}
\newcommand{\iptextcolored}[2]{%
    {\scriptsize\ttfamily\colorbox{#2}{\textcolor{black!70}{#1}}}%
}
...
% Internal: Validate node parameters (name and IP cannot be empty)
\newcommand{\validateNodeParams}[2]{
    \IfStrEq{#1}{}{
        \PackageError{node_definitions}{Node name cannot be empty}{}
    }{}
    \IfStrEq{#2}{}{
        \PackageError{node_definitions}{IP address cannot be empty for node #1}{}
    }{}
...
\newcommand{\createAttacker}[5]{
    \node[attacker] (#1) at (#3,#4) {
        \textbf{#5} \\[2pt]
        \iptextcolored{#2}{threatCritical!20}
    };
}

% TODO: Advanced node creation (future enhancements)
% - Add support for custom node shapes via parameters
% - Create composite nodes (e.g., server rack with multiple servers)
% Note: Node templates, IP validation, and status indicators are implemented
...
\newcommand{\createServerWithPorts}[6]{
    \node[server, minimum height=2cm] (#1) at (#3,#4) {
        \textbf{#5} \\[2pt]
        \iptext{#2} \\[3pt]
        {\tiny\ttfamily\textcolor{black!60}{#6}}
    };
}
...
\newcommand{\createDatabasePrimary}[5]{
    \node[database primary] (#1) at (#3,#4) {
        \textbf{#5} \\[2pt]
        \iptext{#2} \\[1pt]
        {\tiny\bfseries\textcolor{databaseTeal!90}{PRIMARY}}
    };
}
...
\newcommand{\createDatabaseReplica}[5]{
    \node[database replica] (#1) at (#3,#4) {
        \textbf{#5} \\[2pt]
        \iptext{#2} \\[1pt]
        {\tiny\bfseries\textcolor{databaseTeal!70}{REPLICA}}
    };
}
...
\newcommand{\createDatabaseCluster}[5]{
    \node[database cluster] (#1) at (#3,#4) {
        \textbf{#5} \\[2pt]
        \iptext{#2} \\[1pt]
        {\tiny\bfseries\textcolor{databaseTeal!85}{CLUSTER}}
    };
}
...
\newcommand{\createLoadBalancerActive}[6]{
    \node[loadbalancer active] (#1) at (#3,#4) {
        \textbf{#5} \\[2pt]
        \iptext{#2} \\[1pt]
        {\tiny\ttfamily\textcolor{loadBalancerCyan!90}{#6}} \\[1pt]
        {\tiny\bfseries\textcolor{clientGreen!80}{ACTIVE}}
    };
}
...
\newcommand{\createLoadBalancerPassive}[5]{
    \node[loadbalancer passive] (#1) at (#3,#4) {
        \textbf{#5} \\[2pt]
        \iptext{#2} \\[1pt]
        {\tiny\bfseries\textcolor{black!50}{STANDBY}}
    };
}
...
\newcommand{\createVM}[6]{
    \node[vm] (#1) at (#3,#4) {
        \textbf{#5} \\[2pt]
        \iptext{#2} \\[1pt]
        {\tiny\ttfamily\textcolor{vmIndigo!70}{Host: #6}}
    };
}
...
\newcommand{\createHypervisor}[6]{
    \node[vm hypervisor] (#1) at (#3,#4) {
        \textbf{#5} \\[2pt]
        \iptext{#2} \\[3pt]
        {\tiny\bfseries\textcolor{vmIndigo!90}{HYPERVISOR}} \\[1pt]
        {\tiny\ttfamily\textcolor{vmIndigo!70}{VMs: #6}}
    };
}
...
\newcommand{\createVMWithResources}[8]{
    \node[vm] (#1) at (#3,#4) {
        \textbf{#5} \\[2pt]
        \iptext{#2} \\[2pt]
        {\tiny\ttfamily\textcolor{black!60}{CPU: #6 | RAM: #7 | Disk: #8}}
    };
}
...
\newcommand{\createContainer}[6]{
    \node[container] (#1) at (#3,#4) {
        \textbf{#5} \\[1pt]
        \iptext{#2} \\[1pt]
        {\tiny\ttfamily\textcolor{containerBlue!70}{#6}}
    };
    % Add stacked effect (configurable offset)
    \draw[containerBlue!60, line width=0.8pt]
        ([xshift=-\containerStackOffset, yshift=\containerStackOffset]#1.north west) --
        ([xshift=-\containerStackOffset, yshift=\containerStackOffset]#1.north east) --
        ([xshift=-\containerStackOffset]#1.south east);
    \draw[containerBlue!40, line width=0.6pt]
        ([xshift=-2\containerStackOffset, yshift=2\containerStackOffset]#1.north west) --
        ([xshift=-2\containerStackOffset, yshift=2\containerStackOffset]#1.north east) --
        ([xshift=-2\containerStackOffset]#1.south east);
}
...
\newcommand{\createPod}[6]{
    \node[container pod] (#1) at (#3,#4) {
        \textbf{#5} \\[1pt]
        \iptext{#2} \\[1pt]
        {\tiny\ttfamily\textcolor{containerBlue!70}{NS: #6}}
    };
}
...
\newcommand{\createContainerWithPorts}[6]{
    \node[container] (#1) at (#3,#4) {
        \textbf{#5} \\[1pt]
        \iptext{#2} \\[1pt]
        {\tiny\ttfamily\textcolor{containerBlue!70}{Ports: #6}}
    };
    % Add stacked effect (configurable offset)
    \draw[containerBlue!60, line width=0.8pt]
        ([xshift=-\containerStackOffset, yshift=\containerStackOffset]#1.north west) --
...
\newcommand{\createRack}[5]{
    ...
    \foreach \yint in {1,3,...,9} {
        \pgfmathsetmacro{\y}{\yint/10}
        \fill[black!40] ([yshift=\y*10pt]#1.north west) circle (1pt);
        \fill[black!40] ([yshift=\y*10pt]#1.north east) circle (1pt);
    }
}

% TODO: Enhanced clustering features (future enhancements)
% - Auto-arrange nodes within clusters
% Note: Mobile devices, IoT devices, and subnet boundaries are implemented
...
\newcommand{\createDetailedServer}[7]{
    \node[multipart node] (#1) at (#3,#4) {
        ...
        \nodepart{two}
        \iptext{#2} \\
        {\tiny\ttfamily\textcolor{black!70}{#6}}
        \nodepart{three}
        {\tiny\bfseries\textcolor{clientGreen!80}{#7}}
    };
}
...
\newcommand{\createNodeWithServices}[7]{
    \node[
        ...
        \textbf{#5}
        \nodepart{two}
        {\scriptsize\ttfamily\textcolor{black!70}{#2}} \\
        {\tiny\ttfamily\textcolor{serverBlue!80}{Ports: #6}}
        \nodepart{three}
        {\tiny\ttfamily\textcolor{black!60}{#7}}
    };
}
...
\newcommand{\createNodeWithMetrics}[8]{
    \node[
        ...
        \nodepart{four}
        {\tiny\textcolor{black!60}{System Metrics}}
    };
}
...
\newcommand{\createSecurityNode}[8]{
    ...
    \node[
        ...
        \nodepart{two}
        \iptext{#2} \\
        {\tiny\ttfamily\textcolor{black!70}{Vulnerabilities: #6}} \\
        {\tiny\ttfamily\textcolor{black!70}{Max CVSS: #7}}
        \nodepart{three}
        {\tiny\bfseries\textcolor{\statuscolor!90}{\statustext}}
    };
}
...
\newcommand{\createMobilePhone}[6]{
    \node[mobile phone] (#1) at (#3,#4) {
        {\tiny\bfseries #5} \\[1pt]
        {\tiny\ttfamily\textcolor{black!70}{#2}} \\[1pt]
        {\tiny\ttfamily\textcolor{mobileOrange!80}{#6}}
    };
...
\newcommand{\createLaptop}[6]{
    \node[client, minimum width=2.2cm] (#1) at (#3,#4) {
        \textbf{#5} \\[2pt]
        \iptext{#2} \\[1pt]
        {\tiny\ttfamily\textcolor{clientGreen!70}{User: #6}}
    };
...
\newcommand{\createIoTDevice}[6]{
    \node[iot device] (#1) at (#3,#4) {
        {\small\bfseries #5} \\[1pt]
        {\tiny\ttfamily\textcolor{black!70}{#2}} \\[1pt]
        {\tiny\ttfamily\textcolor{iotGreen!80}{#6}}
    };
}
...
\newcommand{\createSensor}[6]{
    \node[sensor] (#1) at (#3,#4) {
        {\tiny\bfseries #5} \\[1pt]
        {\tiny\ttfamily\textcolor{black!60}{#2}} \\[1pt]
        {\tiny\ttfamily\textcolor{iotGreen!80}{#6}}
    };
}
...
\newcommand{\createSmartDevice}[7]{
    \node[iot device, minimum width=2.5cm] (#1) at (#3,#4) {
        {\small\bfseries #5} \\[1pt]
        {\tiny\ttfamily\textcolor{black!70}{#2}} \\[1pt]
        {\tiny\ttfamily\textcolor{iotGreen!70}{#6}} \\[1pt]
        {\tiny\textcolor{black!60}{#7}}
    };
}
...
\newcommand{\createAWSNode}[5]{
    \node[aws node] (#1) at (#2,#3) {
        \textbf{#4} \\[2pt]
        {\tiny\bfseries\textcolor{cloudAWS!90}{AWS}} \\[1pt]
        {\tiny\ttfamily\textcolor{black!70}{#5}}
    };
}
...
\newcommand{\createAzureNode}[5]{
    \node[azure node] (#1) at (#2,#3) {
        \textbf{#4} \\[2pt]
        {\tiny\bfseries\textcolor{cloudAzure!90}{Azure}} \\[1pt]
        {\tiny\ttfamily\textcolor{black!70}{#5}}
    };
}
...
\newcommand{\createGCPNode}[5]{
    \node[gcp node] (#1) at (#2,#3) {
        \textbf{#4} \\[2pt]
        {\tiny\bfseries\textcolor{cloudGCP!90}{GCP}} \\[1pt]
        {\tiny\ttfamily\textcolor{black!70}{#5}}
    };
}
...
\newcommand{\createIPS}[6]{
    \node[ips] (#1) at (#3,#4) {
        \textbf{#5} \\[2pt]
        \iptext{#2} \\[2pt]
        {\tiny\bfseries\textcolor{appliancePurple!90}{#6}}
    };
}
...
\newcommand{\createProxy}[6]{
    \node[proxy] (#1) at (#3,#4) {
        \textbf{#5} \\[2pt]
        \iptext{#2} \\[1pt]
        {\tiny\ttfamily\textcolor{appliancePurple!80}{#6}}
    };
}
...
\newcommand{\createWAF}[6]{
    \node[waf] (#1) at (#3,#4) {
        \textbf{#5} \\[2pt]
        \iptext{#2} \\[2pt]
        {\tiny\bfseries\textcolor{appliancePurple!90}{WAF}} \\[1pt]
        {\tiny\ttfamily\textcolor{black!60}{Rules: #6}}
    };
}
...
\newcommand{\createStorage}[6]{
    \node[storage] (#1) at (#3,#4) {
        \textbf{#5} \\[2pt]
        \iptext{#2} \\[1pt]
        {\tiny\ttfamily\textcolor{storageYellow!90}{#6}}
    };
}
...
\newcommand{\createNAS}[7]{
    \node[nas] (#1) at (#3,#4) {
        \textbf{#5} \\[2pt]
        \iptext{#2} \\[1pt]
        {\tiny\bfseries\textcolor{storageYellow!90}{NAS}} \\[1pt]
        {\tiny\ttfamily\textcolor{black!60}{#6 | #7}}
    };
}
...
\newcommand{\createSAN}[7]{
    \node[storage, minimum height=2.5cm] (#1) at (#3,#4) {
        \textbf{#5} \\[2pt]
        \iptext{#2} \\[2pt]
        {\tiny\bfseries\textcolor{storageYellow!90}{SAN}} \\[1pt]
        {\tiny\ttfamily\textcolor{black!60}{#6}} \\[1pt]
        {\tiny\ttfamily\textcolor{storageYellow!80}{#7}}
    };
}
...
\newcommand{\createWirelessAP}[6]{
    \node[wireless ap] (#1) at (#3,#4) {
        {\small\bfseries #5} \\[2pt]
        \iptext{#2} \\[2pt]
        {\tiny\ttfamily\textcolor{wirelessTeal!80}{SSID: #6}}
    };
    % Add wireless signal indicators
    \foreach \rint in {3, 5, 7} {
        \pgfmathsetmacro{\r}{\rint/10}
...
\newcommand{\createNodeRow}[6]{
    \foreach \i in {1,...,#5} {
        ...
        \pgfmathsetmacro{\ipoctet}{\defaultIPOctetStart + \i - 1}
        \ifthenelse{\equal{#1}{server}}{
            \createServer{#2\i}{#3.\ipoctet}{\xpos}{#4}{#6-\i}
        }{
        \ifthenelse{\equal{#1}{client}}{
            \createClient{#2\i}{#3.\ipoctet}{\xpos}{#4}{#6-\i}
        }{
        \ifthenelse{\equal{#1}{database}}{
            \createDatabase{#2\i}{#3.\ipoctet}{\xpos}{#4}{#6-\i}
        }{
            \createServer{#2\i}{#3.\ipoctet}{\xpos}{#4}{#6-\i}
        }}}
    }
...
\newcommand{\createNodeGrid}[7]{
    ...
    \foreach \row in {1,...,#6} {
        \foreach \col in {1,...,#7} {
            ...
            \pgfmathsetmacro{\nodenum}{\nodenum + 1}
            \pgfmathsetmacro{\ipoctet}{\defaultIPOctetStart + \nodenum}
            \ifthenelse{\equal{#1}{server}}{
                \createServer{#2\nodenum}{#3.\ipoctet}{\xpos}{\ypos}{Node-\nodenum}
            }{
                \createClient{#2\nodenum}{#3.\ipoctet}{\xpos}{\ypos}{Node-\nodenum}
            }
        }
...
\newcommand{\createStarTopology}[5]{
    ...
    \foreach \i in {1,...,#5} {
        ...
        \pgfmathsetmacro{\ipoctet}{\defaultIPOctetStart + \i}

        \ifthenelse{\equal{#3}{client}}{
            \createClient{#1_node\i}{#4.\ipoctet}{\xpos}{\ypos}{Client-\i}
        }{
            \createServer{#1_node\i}{#4.\ipoctet}{\xpos}{\ypos}{Server-\i}
        }
...
\newcommand{\highlightPath}[1]{
    \def\prevnode{} % Initialize to prevent scope issues from previous calls
    \foreach \n [count=\i] in {#1} {
        \ifnum\i>1
    ...
    \foreach \n [count=\i] in {#1} {
        \ifnum\i>1
            ...
            \ifnum\connvalid=0\relax
                \pgfmathtruncatemacro{\tempincrement}{#2 + 1}
                \let#2\tempincrement
            \fi
        \fi
        \global\let\prevnode\n
...
\newcommand{\validatePortRange}[4]{
    ...
    \foreach \p in {#2,...,#3} {
        ...
        \ifnum\inuse=1\relax
            \pgfmathtruncatemacro{\tempincrement}{#4 + 1}
            \let#4\tempincrement
        \fi
    }
}
...
% Note: Handles empty node list gracefully (returns 0)
    ...
    \foreach \n in {#2} {
        ...
        \ifthenelse{\equal{#1}{public}}{
            ...
            \ifnum\isprivate=1\relax
                \pgfmathtruncatemacro{\tempincrement}{#3 + 1}
                \let#3\tempincrement
                \stepcounter{validationWarningCount}
            \fi
        }{
...
\newcommand{\calculateNetworkHealth}[1]{
    ...
    % Deduct for errors
    \pgfmathsetmacro{\score}{\score - (\thevalidationErrorCount * \validationErrorPenalty)}

    % Deduct for warnings
    \pgfmathsetmacro{\score}{\score - (\thevalidationWarningCount * \validationWarningPenalty)}

    % Deduct for IP conflicts (critical)
    \pgfmathsetmacro{\score}{\score - (\theipConflictCount * \ipConflictPenalty)}

    % Deduct for port conflicts
    \pgfmathsetmacro{\score}{\score - (\theportConflictCount * \portConflictPenalty)}
...
\newcommand{\generateDriftReport}[2]{
    \node[
        ...
        \pgfmathtruncatemacro{\totalchanges}{\thenodesAdded + \thenodesRemoved + \thenodesModified}
...
\newcommand{\generateChangeSummary}[2]{
    \pgfmathtruncatemacro{\totalchanges}{\thenodesAdded + \thenodesRemoved + \thenodesModified + \theconnectionsChanged}

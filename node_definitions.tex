...
% Configurable parameters for node styling
...
\newcommand{\validateIPv4Enhanced}[2]{
    ...
    \ifnum\dotcount=3\relax
        ...
        \def\validoctets{1}
        \ifnum\octetA<0\relax \def\validoctets{0}\fi
        \ifnum\octetA>255\relax \def\validoctets{0}\fi
        \ifnum\octetB<0\relax \def\validoctets{0}\fi
        \ifnum\octetB>255\relax \def\validoctets{0}\fi
        \ifnum\octetC<0\relax \def\validoctets{0}\fi
        \ifnum\octetC>255\relax \def\validoctets{0}\fi
        \ifnum\octetD<0\relax \def\validoctets{0}\fi
        \ifnum\octetD>255\relax \def\validoctets{0}\fi

        \ifnum\validoctets=1\relax
            \def#2{1} % Mark as valid if format and range are correct
        \fi
    \fi
}
...
\newcommand{\createNodeRow}[6]{
    \foreach \i in {1,...,#5} {
        ...
        \pgfmathsetmacro{\ipoctet}{10 + \i - 1}
        \ifthenelse{\equal{#1}{server}}{

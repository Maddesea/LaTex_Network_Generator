...
% ============================================================================
% ROUND 5: ADVANCED AUTOMATION & PRODUCTION INTELLIGENCE
% ============================================================================
...
% Curved connection with auto-calculated bezier
\tikzset{
    smooth curve/.style={
        thick,
        draw=#1,
        to path={
            .. controls ($(\tikztostart)!0.5!(\tikztotarget) + (0,1)$)
            and ($(\tikztostart)!0.5!(\tikztotarget) + (0,-1)$)
            .. (\tikztotarget) \tikztonodes
        }
    },
    smooth curve/.default=blue!60
}
...
\tikzset{
    ...
    orthogonal h/.default=blue!60
}

% Smart curved path (avoids center overlap)
\tikzset{
    smart curve/.style={
        thick,
        draw=#1,
    ...
    smart curve/.default=blue!60
}

% Loop connection (self-referencing)
\tikzset{
    loop conn/.style={
        thick,
        draw=#1,
    ...
    loop conn/.default=orange!70
}

% Bundled connection indicator (shows multiple connections as one)
\newcommand{\bundledConnection}[4]{
    % #1=from, #2=to, #3=count, #4=label
    \draw[line width=4pt, draw=blue!40] (#1) -- (#2);
    \draw[line width=3pt, draw=blue!60, ->] (#1) -- (#2)
        node[midway, circle, fill=white, inner sep=2pt, font=\tiny\bfseries] {#3};
    \node[above, font=\scriptsize] at ($(#1)!0.5!(#2)$) {#4};
}

% Smart connection router (chooses best path automatically)
\newcommand{\smartConnection}[3]{
    % #1=from, #2=to, #3=style
    \draw[#3, smart curve] (#1) to (#2);
}
...
% Latency indicator on connection
\newcommand{\latencyIndicator}[3]{
    % #1=from, #2=to, #3=latency in ms
    \draw[thick, draw=blue!60, ->] (#1) -- (#2);
    \node[midway, above, fill=white, inner sep=1pt, font=\tiny] at ($(#1)!0.5!(#2)$) {
        \ifthenelse{#3<10}{\textcolor{green!70!black}}{
        \ifthenelse{#3<50}{\textcolor{yellow!80!black}}{
        \textcolor{red!80!black}}}{\textbf{#3ms}}
    };
}

% Throughput indicator
\newcommand{\throughputIndicator}[3]{
    % #1=from, #2=to, #3=throughput (e.g., "10Gbps", "1Mbps")
    \draw[line width=2pt, draw=cyan!60, ->] (#1) -- (#2);
    \node[midway, below, fill=cyan!20, rounded corners=2pt, inner sep=2pt, font=\tiny\bfseries]
        at ($(#1)!0.5!(#2)$) {⚡ #3};
}

% Packet loss indicator
\newcommand{\packetLossIndicator}[3]{
    % #1=from, #2=to, #3=packet loss %
    \draw[thick, ->] (#1) -- (#2);
    \pgfmathparse{#3>5 ? 1 : 0}
    \ifnum\pgfmathresult=1
        \draw[draw=red!70, line width=1.5pt, dashed] (#1) -- (#2);
        \node[midway, above, fill=red!20, font=\tiny, text=red!80!black]
            at ($(#1)!0.5!(#2)$) {⚠ #3\% loss};
    \fi
}

% Jitter indicator (connection quality)
\tikzset{
    low jitter/.style={
        draw=green!60,
        line width=2pt,
...
\newcommand{\metricsConnection}[5]{
    % #1=from, #2=to, #3=latency, #4=throughput, #5=packet_loss
    \draw[thick, draw=blue!60, ->] (#1) -- (#2);
    \node[above=2pt, font=\tiny, fill=white, inner sep=1pt] at ($(#1)!0.5!(#2)$) {
        \textcolor{blue!70}{#3ms} | \textcolor{cyan!70}{#4}
    };
    \pgfmathparse{#5>1 ? 1 : 0}
    \ifnum\pgfmathresult=1
        \node[below=2pt, font=\tiny, fill=red!20, text=red!80!black]
            at ($(#1)!0.5!(#2)$) {⚠ #5\% loss};
    \fi
}
...
% Attack path visualization (red dashed with steps)
\newcommand{\attackPath}[3]{
    % #1=nodes list (comma-separated), #2=path name, #3=attack type
    \draw[line width=3pt, draw=red!70, dashed, ->]
        plot[smooth] coordinates {#1}
        node[midway, above, fill=red!20, text=red!80!black, font=\tiny\bfseries] {#3};
}

% Blast radius indicator (shows impact zone)
\newcommand{\blastRadius}[2]{
    % #1=center node, #2=radius
    \draw[draw=red!50, fill=red!10, dashed, line width=2pt, opacity=0.6]
        (#1) circle (#2);
    \node[above=0.5cm of #1, fill=red!30, text=red!90!black, font=\tiny\bfseries,
          rounded corners=2pt, inner sep=2pt] {⚠ BLAST RADIUS};
}

% Defense layer (security control boundary)
\tikzset{
    defense layer/.style={
        draw=green!60,
        fill=green!5,
...
\tikzset{
    attack vector/.style={
        ...
        postaction={
            ...
            decoration={
                text along path,
                text={|\tiny\bfseries|ATTACK},
                text color=red!80!black
            }
        }
    }
}
...
\newcommand{\securityControl}[3]{
    % #1=position, #2=control type, #3=effectiveness (%)
    \node[shield, fill=green!60, text=white, font=\tiny\bfseries, minimum size=8mm]
        at #1 {#2};
    \pgfmathparse{#3>70 ? 1 : 0}
    \ifnum\pgfmathresult=1
        \node[above=1pt, fill=green!30, font=\tiny, rounded corners=1pt] at #1 {✓#3\%};
    \else
        \node[above=1pt, fill=orange!30, font=\tiny, rounded corners=1pt] at #1 {⚠#3\%};
    \fi
}

% Threat score indicator on node
\newcommand{\threatScore}[2]{
    % #1=node, #2=score (0-10)
    \pgfmathparse{#2>7 ? int(1) : int(0)}
    \ifnum\pgfmathresult=1
        \node[below right=2pt of #1, fill=red!80, text=white, font=\tiny\bfseries,
              circle, minimum size=6mm] {#2};
    \else
        \pgfmathparse{#2>4 ? int(1) : int(0)}
        \ifnum\pgfmathresult=1
            \node[below right=2pt of #1, fill=orange!80, text=white, font=\tiny\bfseries,
                  circle, minimum size=6mm] {#2};
        \else
            \node[below right=2pt of #1, fill=green!70, text=white, font=\tiny\bfseries,
                  circle, minimum size=6mm] {#2};
        \fi
    \fi
}
...
\newcommand{\zeroTrustTemplate}{
    % Identity layer
    \node[rectangle, draw=purple!70, fill=purple!10, minimum width=12cm,
          minimum height=2cm, font=\bfseries] (identity) at (0,6) {Identity \& Access Layer};

    % Micro-segmentation zones
    \node[zero trust, minimum width=3cm, minimum height=3cm] (zone1) at (-4,2) {};
    \node[zero trust, minimum width=3cm, minimum height=3cm] (zone2) at (0,2) {};
    \node[zero trust, minimum width=3cm, minimum height=3cm] (zone3) at (4,2) {};

    % Policy enforcement points
    \node[diamond, fill=purple!60, text=white, font=\tiny] (pep1) at (-4,4) {PEP};
    \node[diamond, fill=purple!60, text=white, font=\tiny] (pep2) at (0,4) {PEP};
    \node[diamond, fill=purple!60, text=white, font=\tiny] (pep3) at (4,4) {PEP};

    % Connections
    \draw[thick, purple!60, ->] (identity) -- (pep1);
    \draw[thick, purple!60, ->] (identity) -- (pep2);
    \draw[thick, purple!60, ->] (identity) -- (pep3);
    \draw[thick, purple!60, ->] (pep1) -- (zone1);
    \draw[thick, purple!60, ->] (pep2) -- (zone2);
    \draw[thick, purple!60, ->] (pep3) -- (zone3);
}

% SASE Architecture Template
\newcommand{\saseTemplate}{
    % Cloud security edge
    \node[cloud, fill=cyan!30, minimum width=8cm, minimum height=3cm, font=\bfseries]
        (sase) at (0,5) {SASE Cloud\\(Security \& Networking)};

    % Edge locations
    \node[container node] (edge1) at (-6,2) {Edge\\PoP 1};
    \node[container node] (edge2) at (0,2) {Edge\\PoP 2};
    \node[container node] (edge3) at (6,2) {Edge\\PoP 3};

    % Users and branches
    \node[mobile device] (user1) at (-6,-1) {Mobile};
    \node[client premium] (branch) at (0,-1) {Branch};
    \node[iot device] (iot) at (6,-1) {IoT};

    % Services in SASE
    \node[above left=0.5cm of sase, font=\tiny] {ZTNA | SWG | CASB | FWaaS};

    % Connections
    \draw[encrypted conn] (user1) -- (edge1);
    \draw[encrypted conn] (branch) -- (edge2);
    \draw[encrypted conn] (iot) -- (edge3);
    \draw[vpn tunnel] (edge1) -- (sase);
    \draw[vpn tunnel] (edge2) -- (sase);
    \draw[vpn tunnel] (edge3) -- (sase);
}

% SD-WAN Architecture Template
\newcommand{\sdwanTemplate}{
    % SD-WAN Controller
    \node[cloud service] (controller) at (0,6) {SD-WAN\\Controller};

    % Branch sites
    \node[router premium] (branch1) at (-6,2) {Branch 1\\SD-WAN};
    \node[router premium] (branch2) at (0,2) {Branch 2\\SD-WAN};
    \node[router premium] (branch3) at (6,2) {Branch 3\\SD-WAN};

    % Transport networks
    \node[cloud, fill=blue!20] (mpls) at (-6,-2) {MPLS};
    \node[cloud, fill=green!20] (internet) at (0,-2) {Internet};
    \node[cloud, fill=orange!20] (lte) at (6,-2) {LTE/5G};

    % Control plane
    \draw[dashed, blue!60, ->] (controller) -- (branch1);
    \draw[dashed, blue!60, ->] (controller) -- (branch2);
    \draw[dashed, blue!60, ->] (controller) -- (branch3);

    % Data plane - multiple paths
    \draw[thick, green!60] (branch1) -- (mpls);
    \draw[thick, green!60] (branch1) to[bend right=15] (internet);
    \draw[thick, blue!60] (branch2) -- (internet);
    \draw[thick, blue!60] (branch2) to[bend right=15] (mpls);
    \draw[thick, orange!60] (branch3) -- (lte);
    \draw[thick, orange!60] (branch3) to[bend left=15] (internet);

    % Mesh connections
    \draw[load balanced, cyan!50] (branch1) to[bend left=20] (branch2);
    \draw[load balanced, cyan!50] (branch2) to[bend left=20] (branch3);
}

% Hybrid Cloud Architecture Template
\newcommand{\hybridCloudTemplate}{
    % On-premises
    \node[datacenter zone, minimum width=5cm, minimum height=4cm] (onprem) at (-4,0) {};
    \node[above=0.2cm] at (onprem.north) {\textbf{On-Premises}};
    \node[server premium] (dc1) at (-5,0) {DC};
    \node[database cluster] (db1) at (-3,0) {DB};

    % Public Cloud
    \node[cloud zone, minimum width=5cm, minimum height=4cm] (cloud) at (4,0) {};
    \node[above=0.2cm] at (cloud.north) {\textbf{Public Cloud}};
    \node[cloud service] (vm1) at (3,0.5) {Compute};
    \node[container node] (k8s) at (5,0.5) {K8s};
    \node[cloud service] (storage) at (4,-0.8) {Storage};

    % Hybrid connectivity
    \draw[vpn tunnel, line width=3pt] (onprem.east) -- (cloud.west)
        node[midway, above, fill=white, font=\small\bfseries] {VPN / Direct Connect};

    % Replication
    \draw[replication] (db1) to[bend left=20] (storage);
    \draw[sync] (dc1) to[bend left=20] (vm1);
}
...
% Auto-sized node (adjusts to content)
\tikzset{
    auto node/.style={
        base node,
        text width=,
...
% Smart server (auto-sizes based on label length)
\newcommand{\smartServer}[4]{
    % #1=name, #2=x, #3=y, #4=label
    \node[server premium, auto node] (#1) at (#2,#3) {#4};
}

% Smart client
\newcommand{\smartClient}[4]{
    % #1=name, #2=x, #3=y, #4=label
    \node[client premium, auto node] (#1) at (#2,#3) {#4};
}
...
\newcommand{\batchStyle}[2]{
    % #1=node list (comma-separated), #2=style
    \foreach \n in {#1} {
        \tikzset{\n/.append style={#2}}
    }
}

% Add badge to multiple nodes
\newcommand{\batchBadge}[3]{
    % #1=node list, #2=badge style, #3=position
    \foreach \n in {#1} {
        \nodeBadge{\n}{#2}{#3}{✓}
    }
}
...
\newcommand{\highlightNodes}[2]{
    % #1=node list, #2=color
    \foreach \n in {#1} {
        \draw[line width=3pt, draw=#2, opacity=0.5] (\n.north west) rectangle (\n.south east);
    }
}

% Connect all nodes in a list
...
% Show added node (green highlight)
\tikzset{
    added node/.style={
        base node,
        draw=green!70,
        line width=3pt,
        fill=green!10,
        label={[fill=green!30, text=green!80!black, font=\tiny]:NEW}
    }
}

% Show removed node (red highlight with strikethrough)
\tikzset{
    removed node/.style={
        base node,
        draw=red!70,
        ...
        fill=red!10,
        opacity=0.5,
        label={[fill=red!30, text=red!80!black, font=\tiny]:REMOVED}
    }
}

% Show modified node (orange highlight)
\tikzset{
    modified node/.style={
        base node,
        draw=orange!70,
        line width=3pt,
        fill=orange!10,
        label={[fill=orange!30, text=orange!80!black, font=\tiny]:MODIFIED}
    }
}

% Change indicator
\newcommand{\changeIndicator}[3]{
    % #1=node, #2=change type (added/removed/modified), #3=description
    \node[below=0.5cm of #1, fill=yellow!20, rounded corners=2pt, font=\tiny,
...
% Diff legend
\newcommand{\diffLegend}[2]{
    % #1=x, #2=y
    \node[fill=white, draw=black, line width=1pt, rounded corners=3pt, inner sep=5pt]
        at (#1,#2) {
        \begin{tabular}{ll}
        \textcolor{green!70}{■} & Added \\
        \textcolor{orange!70}{■} & Modified \\
        \textcolor{red!70}{■} & Removed
        \end{tabular}
    };
}

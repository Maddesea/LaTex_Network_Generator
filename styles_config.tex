...
%
% REQUIRED TIKZ LIBRARIES (must be loaded in your main document):
% \usetikzlibrary{shapes.geometric}    % For cylinder, star, diamond, cloud, tape shapes
% \usetikzlibrary{shapes.multipart}    % For rectangle split nodes
% \usetikzlibrary{patterns}            % For pattern fills (firewall, accessibility)
% \usetikzlibrary{decorations.markings} % For connection markers
% \usetikzlibrary{decorations.pathmorphing} % For wireless/wavy connections
% \usetikzlibrary{shadows.blur}        % For blur shadow effects
% \usetikzlibrary{shadings}            % For gradient fills (CRITICAL for gradient styles)
% \usetikzlibrary{arrows.meta}         % For arrow specifications
%
% RECOMMENDED PACKAGES:
% \usepackage{xcolor}                  % For extended color support
% \usepackage{ifthen}                  % For conditional logic in themes
...
% Note: Colors are defined directly using \definecolor{name}{RGB}{r,g,b} syntax
...
% Note: Optional parameter [size] is defined for future scalability but currently unused
% Icons are scaled via the tikzpicture scale option (currently 0.5)
...
\newcommand{\phoneIcon}[1][0.4cm]{%
    \begin{tikzpicture}[scale=0.5, baseline=-0.5ex]
        \draw[black!70, fill=black!70, rounded corners=1pt] (-0.15,0) rectangle (0.15,0.6);
...
% These styles are helper templates - use with label= option on nodes
% Example: \node[server, label={[badge windows, anchor=south west, xshift=0.8cm, yshift=0.4cm]north east:Win}] {Server};
\tikzset{
    top right badge/.style n args={2}{
        label={[#1, anchor=south west, xshift=0.8cm, yshift=0.4cm]north east:#2}
    },
    top left badge/.style n args={2}{
        label={[#1, anchor=south east, xshift=-0.8cm, yshift=0.4cm]north west:#2}
    },
    bottom right badge/.style n args={2}{
        label={[#1, anchor=north west, xshift=0.8cm, yshift=-0.4cm]south east:#2}
    },
    bottom left badge/.style n args={2}{
        label={[#1, anchor=north east, xshift=-0.8cm, yshift=-0.4cm]south west:#2}
    }
}

% Predefined badge combinations
% Note: These create positioned nodes and must be used inside a tikzpicture environment
% Recommended usage: Use badge styles directly with node labels instead
% Example: \node[server, label={[badge windows]above:Win}] {Server};

% Helper macros (use within tikzpicture with explicit positioning)
\newcommand{\windowsBadge}[1][(0,0)]{%
    \node[badge windows] at #1 {Win};
}

\newcommand{\linuxBadge}[1][(0,0)]{%
    \node[badge linux] at #1 {Linux};
}

\newcommand{\macOSBadge}[1][(0,0)]{%
    \node[badge macos] at #1 {Mac};
}

\newcommand{\onlineBadge}[1][(0,0)]{%
    \node[badge online] at #1 {●};
}

\newcommand{\offlineBadge}[1][(0,0)]{%
    \node[badge offline] at #1 {●};
}
...
% Note: Custom style templates can be manually saved by copying style definitions
% to a separate .tex file and loading via \input{} command
...
% Valid range for #1: 0-100 (values outside this range not validated)
...
% Usage: Place inside tikzpicture environment with explicit position
% Example: \basicLegend{(-2,4)} places legend at coordinates (-2,4)

\newcommand{\basicLegend}[1][(0,0)]{%
    \node[legend box] at #1 {
        \begin{tabular}{ll}
            \legendTitle{Legend} \\
            \legendServer \\
...
\newcommand{\connectionLegend}[1][(0,0)]{%
    \node[legend box] at #1 {
        \begin{tabular}{ll}
...
% IMPORTANT: This command requires 'remember picture' option in tikzpicture environment
% Usage: \begin{tikzpicture}[remember picture] ... \timestamp ... \end{tikzpicture}
\newcommand{\timestamp}{
...
% Usage: \threeTierTemplate{w1}{w2}{w3}{a1}{a2}{a3}{d1}{d2}{d3}
% Creates a 3-tier network with web, app, and data tiers at fixed positions
% Note: Uses hardcoded positions; adjust coordinates in command if needed
...
% Usage: \hubSpokeTemplate{hub_name}{num_spokes}{radius}{hub_style}{spoke_style}
% Example: \hubSpokeTemplate{Router1}{6}{4cm}{router}{client}
...
% Usage: \meshTemplate{num_nodes}{radius}{node_style}
% Example: \meshTemplate{5}{3cm}{server}
% Creates a full mesh network with all nodes connected to each other
